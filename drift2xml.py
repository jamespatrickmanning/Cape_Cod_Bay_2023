...
if (sys.argv[1]=='drift_whs_2022_1') or (sys.argv[1]=='drift_X') or (sys.argv[1]=='drift_fhs_2023_2'): # here is where we are adding miniboat track from Cassie
    ...
    elif (sys.argv[1]=='drift_fhs_2023_2') or (sys.argv[1]=='drift_X'):
        df1=pd.read_csv('https://educationalpassages.org/wp-content/uploads/csv/Riptide_8.csv')
        df2=pd.read_csv('https://educationalpassages.org/wp-content/uploads/csv/Rock_Star_3.csv')
        
        df3=pd.read_csv('https://educationalpassages.org/wp-content/uploads/csv/Riptide_9.csv')
        df4=pd.read_csv('https://educationalpassages.org/wp-content/uploads/csv/Rock_Star_4.csv')
    ...
    for k in range(len(df)):
        if (df['deployment_id'][k]==220410701):
           ...
           esn.append('9999998')
        elif df['deployment_id'][k]==230420703:
            df['deployment_id'][k]='230410703' 
            esn.append('9999997')
        elif df['deployment_id'][k]==230420701:
            df['deployment_id'][k]='230410704'
            esn.append('9999999')
        elif df['deployment_id'][k]==230410703:
            df['deployment_id'][k]='230410709' 
            esn.append('9999997')
        elif df['boat_name'][k]=='Rock_Star_4':
            df['deployment_id'][k]='2304107010'
            esn.append('9999999')
    ...
    # here is where we add makerbuoy data
    if (sys.argv[1]=='drift_fhs_2023_2') or (sys.argv[1]=='drift_X'):
        df1=pd.read_csv('/home/user/drift/data/EPMBD_084_1.csv')
        df2=pd.read_csv('/home/user/drift/data/EPMBD_085_1.csv')
        df=pd.concat([df1,df2],ignore_index=True)
    df['time_stamp']=pd.to_datetime(df['time_stamp'])
    for k in range(len(df)):
        if df['name'][k]=='EP-R084':
            df['name'][k]='230410705' 
            esn.append('9999991')
        elif df['name'][k]=='EP-R085':
            df['name'][k]='230410706'
            esn.append('9999992')
        idss.append(df['name'].values[k])
        yeardays_all.append(float(df['time_stamp'][k].timetuple().tm_yday)+df['time_stamp'][k].hour/24.+df['time_stamp'][k].minute/60/24.)
        lat_all.append(float(df['LAT'][k]))
        lon_all.append(float(df['LON'][k]))
        day_all.append(df['time_stamp'][k].day)
        mth_all.append(df['time_stamp'][k].month)
        hr_all.append(df['time_stamp'][k].hour)
        mn_all.append(df['time_stamp'][k].minute)
        dep_all.append(-0.1)
        nan_all.append('nan')
    ...
    elif sys.argv[1]=='drift_fhs_2023_2':
        dfcsv.to_csv(filedirection+'drift_fhs_2023_2.csv')
    else:
...
for k in range(len(ids)): # loop through all the distinct deployment ids
    ...
    if time[-1]>maxtime:    
      ...
      if ids[k]=='195360601':
          ...
          schoolName='Seton_Hall_Prep'
          depth=0.2      
      elif ids[k]=='230410703':
          boatName='Riptide'
          schoolName='Falmouth_Junior_High'
          depth=0.1
      elif ids[k]=='230410704':
          boatName='Rockstar'
          schoolName='Falmouth_Junior_High'
          depth=0.1      
      elif ids[k]=='230410705':
          boatName='MakerBuoy_drifter'
          schoolName='Falmouth_Junior_High'
          depth=0.2
      elif ids[k]=='230410706':
          boatName='MakerBuoy_drifter'
          schoolName='Falmouth_Junior_High'
          depth=0.2
          
      elif ids[k]=='230410709':
              boatName='Riptide'
              schoolName='Falmouth_Junior_High'
              depth=0.1
      elif ids[k]=='2304107010':
              boatName='Rockstar'
              schoolName='Falmouth_Junior_High'
              depth=0.1
